...
export {${ENV}_GATEWAY_CONTROLLER_ID_NAME,GATEWAY_CONTROLLER_ID_NAME}=$(az deployment group show -g $RESOURCE_GROUP -n $IDENTITIES_DEPLOYMENT_NAME --query properties.outputs.appGatewayControllerIdName.value -o tsv) && \
...
az deployment group create -g $RESOURCE_GROUP --name $DEPLOYMENT_NAME-${env} --template-file ${PROJECT_ROOT}/azuredeploy.json \
...
export ${ENV}_GATEWAY_SUBNET_PREFIX=$(az deployment group show -g $RESOURCE_GROUP -n $DEPLOYMENT_NAME-${env} --query properties.outputs.appGatewaySubnetPrefix.value -o tsv)
export {${ENV}_VNET_NAME,VNET_NAME}=$(az deployment group show -g $RESOURCE_GROUP -n $DEPLOYMENT_NAME-${env} --query properties.outputs.aksVNetName.value -o tsv)
export {${ENV}_CLUSTER_SUBNET_NAME,CLUSTER_SUBNET_NAME}=$(az deployment group show -g $RESOURCE_GROUP -n $DEPLOYMENT_NAME-${env} --query properties.outputs.aksClusterSubnetName.value -o tsv)
export {${ENV}_CLUSTER_SUBNET_PREFIX,CLUSTER_SUBNET_PREFIX}=$(az deployment group show -g $RESOURCE_GROUP -n $DEPLOYMENT_NAME-${env} --query properties.outputs.aksClusterSubnetPrefix.value -o tsv)
export {${ENV}_CLUSTER_FQDN,CLUSTER_FQDN}=$(az deployment group show -g $RESOURCE_GROUP -n $DEPLOYMENT_NAME-${env} --query properties.outputs.aksFqdn.value -o tsv)
export {${ENV}_CLUSTER_NAME,CLUSTER_NAME}=$(az deployment group show -g $RESOURCE_GROUP -n $DEPLOYMENT_NAME-${env} --query properties.outputs.aksClusterName.value -o tsv) && \
...
export DELIVERY_REDIS_HOSTNAME=$(az deployment group show -g $RESOURCE_GROUP -n $DEPLOYMENT_NAME-${env} --query properties.outputs.deliveryRedisHostName.value -o tsv)
export DELIVERY_REDIS_HOSTNAMES=${DELIVERY_REDIS_HOSTNAMES}\'${DELIVERY_REDIS_HOSTNAME}\',
done

# Restrict cluster egress traffic
export FIREWALL_PIP_NAME=$(az deployment group show -g $RESOURCE_GROUP -n $DEPLOYMENT_NAME-${env} --query properties.outputs.firewallPublicIpName.value -o tsv) && \
...
export GATEWAY_CONTROLLER_PRINCIPAL_RESOURCE_ID=$(az deployment group show -g $RESOURCE_GROUP -n ${!IDENTITIES_DEPLOYMENT_NAME_VARIABLE} --query properties.outputs.appGatewayControllerPrincipalResourceId.value -o tsv) && \
export GATEWAY_CONTROLLER_ID_NAME=$(az deployment group show -g $RESOURCE_GROUP -n ${!IDENTITIES_DEPLOYMENT_NAME_VARIABLE} --query properties.outputs.appGatewayControllerIdName.value -o tsv) && \
...
export APP_GATEWAY_NAME=$(az deployment group show -g $RESOURCE_GROUP -n $DEPLOYMENT_NAME-${env} --query properties.outputs.appGatewayName.value -o tsv)
export {${ENV}_APP_GATEWAY_PUBLIC_IP_FQDN,APP_GATEWAY_PUBLIC_IP_FQDN}=$(az deployment group show -g $RESOURCE_GROUP -n $DEPLOYMENT_NAME-${env} --query properties.outputs.appGatewayPublicIpFqdn.value -o tsv)
